...
c
	...
	double precision resolut(3)
        ...
        character*80 inputfile,outdir,grnfile0(6),reports(6)
	character*160 grnfile(6)
...
c
	...
	print *,'#                                                    #'
	print *,'#                                                    #'
	print *,'#     EEEEE   FFFFF    GGGG   RRRR    N   N          #'
	print *,'#     E       F       G       R   R   NN  N          #'
	print *,'#     EEEE    FFFF    G GGG   RRRR    N N N          #'
	print *,'#     E       F       G   G   R R     N  NN          #'
	print *,'#     EEEEE   F        GGGG   R  R    N   N          #'
	print *,'#                                                    #'
	print *,'#                                                    #'
	print *,'#             Single forces version by               #'
	print *,'#                 Massimo Nespoli                    #'
	print *,'#             massimo.nespoli2@unibo.it              #'
	print *,'#              last update: June 2021                #' 
	print *,'#                                                    #'
	print *,'#                                                    #'
	print *,'#           first implemented: Nov. 2001             #'
	print *,'#                      by                            #'
...
c
	call getdata(10,dataline)
        read(dataline,*)outdir,(grnfile0(i),i=1,6)
c
	len=index(outdir,' ')-1
c
	if(len.lt.1)then
...
c
	...
	reports(4)='Source type: vertical single force fz'
	reports(5)='Source type: horizontal single force fx'
	reports(6)='Source type: horizontal single force fy'
...
c	  if(istype.le.2)then
c	    write(unit,'(a)')'#   uz[m]         ur[m]         ut[m] '
c     &        //'        ezz           err           ett   '
c     &        //'        ezr           ert           etz   '
c     &        //'        duz/dr'
c	  else
c	    write(unit,'(a)')'#   uz[cm]        ur[cm]'
c     &        //'        ezz           err           ett   '
c     &        //'        ezr           duz/dr'
c	  endif
	  write(unit,'(a)')'#'
...
c
	  call edgbstab(6-istype)
c
	  do izs=1,nzs
	    zs=zs1+dble(izs-1)*dzs
            call edglayer(zs,ls,zrec,lzrec,h,n0)
c	    moment=ro(nno(ls))*vs(nno(ls))*vs(nno(ls))
        moment=1.0
...
c	    if(istype.le.2)then
c	      do j=1,nr
c	        write(unit,'(10E14.6)')(u(i,j),i=1,10)
c	      enddo
c	    else
c	      do j=1,nr
c	        write(unit,'(10E14.6)')(u(i,j),i=1,2),(u(i,j),i=4,7)
c    &                               ,u(10,j)
c	      enddo
c	    endif
	  enddo
	...
	print *,'######################################################'
	print *,'#                                                    #'
	print *,'#        Successful computations with EFGRN          #'
	print *,'#                                                    #'
	print *,'######################################################'
