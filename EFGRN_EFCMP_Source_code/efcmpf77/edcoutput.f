...
c
	  ...
	  write(31,'(a)')'# Displacements calculated with efcmp'
...
c
	  ...
	  write(32,'(a)')'# Strains calculated with efcmp'
...
c
	  ...
	  write(33,'(a)')'# Stresses calculated with efcmp'
...
c
c	if(iouts(4).eq.1)then
c	  print *,'... output 2 tilt components ...'
c
c	  outfile=outdir(1:lendir)//outputs(4)
c	  open(34,file=outfile,status='unknown')
c	  write(34,'(a)')'# Vertical tilts calculated with efcmp'
c	  write(34,'(a)')'# The input data file is '//infile(1:leninf)
c	  title='#   X_m         Y_m         dUx/dz      dUy/dz'
c	  write(34,'(a46)')title
c	  do irec=1,nrec
c	    write(34,1001)xrec(irec),yrec(irec),(tilt(irec,i),i=1,2)
c	  enddo
c	  close(34)
c	endif
1001	format(8E12.4)
